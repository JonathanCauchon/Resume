    ...
    **Class Attributes**: Are calculated by the different member functions during simulation.
    ...
    def getPropConstants(self):
        ...
        if self.polyfit_file is None:
            ...
            n1 = np.reshape(np.loadtxt(
                r"./Database/neff/neff_1.txt"), (5, 5, 5))
            n2 = np.reshape(np.loadtxt(
                r"./Database/neff/neff_2.txt"), (5, 5, 5))
            ...
            self.n1_profile = neffThermal + \
                scipy.interpolate.interpn(w1_w2_wvl, n1, d)
            self.n2_profile = neffThermal + \
                scipy.interpolate.interpn(w1_w2_wvl, n2, d)
            ...
            with open(self.polyfit_file, "r") as f:
                ...
                self.n1_profile = neffThermal + \
                    float(a1)*wavelength_tiled + float(b1)
                self.n2_profile = neffThermal + \
                    float(a2)*wavelength_tiled + float(b2)
                self.beta1_profile = 2*math.pi / wavelength_tiled * self.n1_profile
                self.beta2_profile = 2*math.pi / wavelength_tiled * self.n2_profile
    ...
    def getChirpProfile(self):
        ...
        if self.polyfit_file is None:
            ...
            if self.w1_profile is None:
                ...
                self.w1_profile = np.linspace(
                    self.w1[0], self.w1[-1], self.N_seg)
                self.w1_profile = np.round(
                    self.w1_profile/self.w_chirp_step)*self.w_chirp_step
            ...
            if self.w2_profile is None:
                ...
                self.w2_profile = np.linspace(
                    self.w2[0], self.w2[-1], self.N_seg)
                self.w2_profile = np.round(
                    self.w2_profile/self.w_chirp_step)*self.w_chirp_step
    ...
    def propagate(self):
        ...
        S2[:, :, 3, 0] = 1j*np.conj(kappa_12) * \
            np.exp(-1j*(beta_del_1+beta_del_2)*l_cum)

        S2[:, :, 0, 1] = 0
        S2[:, :, 1, 1] = -1j*beta_del_2
        S2[:, :, 2, 1] = 1j*np.conj(kappa_12) * \
            np.exp(-1j*(beta_del_1+beta_del_2)*l_cum)
        ...
        l_seg = np.expand_dims(np.expand_dims(l_seg, 2), 3)
